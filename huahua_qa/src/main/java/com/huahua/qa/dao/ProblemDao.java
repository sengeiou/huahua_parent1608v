...
public interface ProblemDao extends JpaRepository<Problem,String>,JpaSpecificationExecutor<Problem>{
     ...
     * @return
     */
    @Query(nativeQuery = true,value = "select * from tb_problem ,tb_pl where 1= 1 " +
            "and  id = problemid " +
            " and  labelid = ? order by visits desc")
     ...
     */
    @Query(nativeQuery = true,value = "select * from tb_problem ,tb_pl where 1= 1 " +
            "and  id = problemid and  reply = 0 " +
            " and  labelid = ? order by createtime desc")
